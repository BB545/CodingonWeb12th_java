...
// - 클래스 이름은 첫ㅁㄴ자를 대문자로 잓ㅇ하는 것이 관례 (PascalCase 방식)
// - 클래스의 구성요소 (속성, 메서드, ...)는 클래스 블록({}) 안에 위치
// - ⭐ 소스 파일 저장시 파일명과 클래스명이 반드시 일치해야 함 (단. 한 파일에 클래스가 여러 개 있는 경우는 제외)

//Main 클래스 (시작 클래스) 정의
// - main 메서드를 가짐
// - Main 클래스는 한 개로 메인 클래스로부터 프로그램이 시작됨 (node에서 진입점 파일같은 역할)
public class Main {
    // Java의 주석
    // 1. // 한 줄
    // 2. /* 여러 줄 */
    ...
    // - 어플리케이션 프로그램이 실행되려면 최소 1개 존재해야함
    // - 프로그램의 진입점으로 JVM에 의해 최초 호출 -> 메인 메소드 블록 내부에 기술 된 명령문들을 순차적으로 실행
