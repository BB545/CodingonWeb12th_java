...
//      => 동일한 객체 중복저장 가능, 이는 동일한 번지수가 저장되는 것을 의미
//      => null 도 저장할 수 있음
...
public class ArrayListEx {
    // 작성 방법
    // 1) List<E> list = new ArrayList<E>(); // E에 지정된 타입의 객체만 지정 가능, generic
    // 2) List<E> list = new ArrayList<>(); // 1번의 간소화된 문법
    // 3) List list = new ArrayList(); // 모든 타입의 객체 저장 -> 타입 안전성 보장되지 않음

    // 참고. 변수 타입 선언 시 List, ArrayList 모두 사용 가능
    // - 같은 결과를 의미하지만, List를 이용해 ArrayList를 생성하면
    //      => "다형성(하나의 객체가 여러 타입을 가짐)"을 이용하므로 유연함
    // 4) ArrayList<E> list = new ArrayList<E>();

    // ex.
    // 도형 shape = new 삼각형(); => shape의 타입은 도형 - 다양한 도형으로 받을 수 있음
    // 삼각형 shape = new 삼각형(); => shape의 타입이 삼각형 - 오로지 삼각형만 받을 수 있음
    ...
    public static void main(String[] args) {
        ...
        // 4. 요소 수정
        numbers.set(1, 77);
        System.out.println("numbers = " + numbers); // 결과 - numbers = [10, 77, 30, 40, 50]
        ...
        System.out.println("numbers = " + numbers); // 결과 - numbers = [10, 7, 77, 30, 40, 50, 100, 99, 98]

        // 7. 요소 위치 찾기
        System.out.println(numbers.indexOf(10)); // 결과 - 0 인덱스에 인자값이 존재함
        System.out.println(numbers.indexOf(100)); // 결과 - 6 인덱스에 인자값이 존재함
        System.out.println(numbers.indexOf(5)); // 결과 - -1 인자값이 ArrayList에 존재하지 않음
        ...
        students.remove(2);
        System.out.println("students = " + students);
        students.remove(2); // 제거 후 앞으로 땡겨진 인원의 인덱스 2번이 다시 지워짐
        // 위 동작으로 3 -> 2 인덱스로 변경되어 삭제됨
        System.out.println("students = " + students);
    }
