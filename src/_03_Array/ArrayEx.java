...
// 배열
// - 배열에 저장할 자료형을 선언하고, 배열 이름과 크기를 선언해야함
// - 배열의 원소는 모두 같은 타입
...
public class ArrayEx {
    ...
    public static void main(String[] args) {
        ...
        // ex. 타입[] 변수 = null;
        // - 배열 변수가 null 인 상태에서 인덱싱으로 원소에 접근하려고 한다면, NullPointerException 에러 발생
        String[] temp = null;
        System.out.println("temp = " + temp);
        // System.out.println("temp[0] = " + temp[0]); // 결과 - NullPointerException 에러
        ...
        // 배열 변수 선언 시점과 값 목록을 대입하는 시점이 다르다면, "new 타입[]"을 중괄호 앞에 붙여주어야함
        charArray = new char[]{ 'A', 'B', 'C' }; // 이렇게 해야함
        System.out.println("charArray[0] = " + charArray[0]);
        System.out.println("charArray[1] = " + charArray[1]);
        System.out.println("charArray[2] = " + charArray[2]);

        // # new 연산자로 배열 선언하고 배열 생성
        // : new 연산자로 배열 처음 생성하면 배열 항목은 기본 값으로 초기화
        ...
        // - 만약, 배열 길이를 벗어난다면?
        // System.out.println(doubleArray[doubleArray.length]); // 결과 - ArrayIndexOutOfBoundsException 에러

        // 배열 (원소) 출력
        // - Arrays 클래스에 내장되어 있는 toString() 메서드 사용
        ...
        langs[0] = "Java";
        langs[1] = "Java";
        langs[2] = new String("Java"); // 동적 할당 - 내용은 같지만 번지는 다르다

        System.out.println("langs = " + Arrays.toString(langs));
        System.out.println(langs[0] == langs[1]); // 결과 - true : 같은 객체 참조
        System.out.println(langs[1] == langs[2]); // 결과 - false : 서로 다른 객체 참조
        System.out.println(langs[1].equals(langs[2])); // 결과 - true: 내용 비교
        System.out.println();

        // 배열 복사
        // - 배열은 크기가 "고정" 되어 있음
        // -> 더 많은 저장 공간이 필요하다면 더 큰 길이의 배열을 새로 만들어서 기존 배열을 복사해야함
        ...
        // 결과 - originFruits = [apple, banana, kiwi]
        // System.out.println("newFruits = " + Arrays.toString(newFruits));
        // 결과 - newFruits = [null, apple, banana, kiwi, null]

        System.arraycopy(originFruits, 1, newFruits, 2, originFruits.length-1);
        System.out.println("originFruits = " + Arrays.toString(originFruits));
        // 결과 - originFruits = [apple, banana, kiwi]
        System.out.println("newFruits = " + Arrays.toString(newFruits));
        // 결과 - newFruits = [null, null, banana, kiwi, null]

        // Arrays 메소드

        // 1. copyOf(arr, copyArrayLength) 메소드 (처음부터 원하는 갯수 복사)
        ...
        System.out.println("copiedIIntArr = " + Arrays.toString(copiedIntArr));
        // 결과 - copiedIIntArr = [1, 2, 3]

        // 2. copyOfRange(arr, sIdx, eIdx) 메소드 (원하는 범위에서 복사)
        int[] rangeArr = Arrays.copyOfRange(originIntArr, 1, 4);
        System.out.println("rangeArr = " + Arrays.toString(rangeArr));
        // 결과 - rangeArr = [2, 3, 4]

        // 3. fill(arr, n) 메소드 (지정한 숫자가 모든 필드에 들어감)
        int[] filledArr = new int[5];
        System.out.println("filled Array (before) = " + Arrays.toString(filledArr));
        // 결과 - filled Array (before) = [0, 0, 0, 0, 0]
        Arrays.fill(filledArr, 7);
        System.out.println("filled Array (after) = " + Arrays.toString(filledArr));
        // 결과 - filled Array (after) = [7, 7, 7, 7, 7]

        // 4. sort(arr) 메소드 (정렬)
        int[] unsortedArr = { 6, 4, 9, 5, 1 };
        Arrays.sort(unsortedArr);
        System.out.println("Sorted Array = " + Arrays.toString(unsortedArr));
        // 결과 - Sorted Array = [1, 4, 5, 6, 9]
        
        // 5. equals() 메소드 (배열의 내용이 같은지 확인) -> 다차원 배열에서는 사용할 수 없음. 다른 메서드 사용
        int[] array1 = { 1, 2, 3 };
        int[] array2 = { 1, 2, 3 };
        int[] array3 = { 4, 2, 3 };
        boolean arraysEqual = Arrays.equals(array1, array2);
        System.out.println("arraysEqual (1 vs 2) = " + arraysEqual); // 결과 - true
        boolean arraysEqual2 = Arrays.equals(array1, array3);
        System.out.println("arraysEqual2 (1 vs 3) = " + arraysEqual2); // 결과 - false

        // 참고. == 비교 연산자 사용하면 -> 주소값은 다 다르게 가지므로 모두 false (참조값을 비교하는 상황)
        System.out.println("Arrays == (1 vs 2) = " + (array1 == array2)); // 결과 - false
        System.out.println("Arrays == (1 vs 3) = " + (array1 == array3)); // 결과 - false
        
        // 6. deepEauqls(arr1, arr2) 메소드 : 다차원 배열의 내용 비교
        ...
        System.out.println("deepArrayEqual (1 vs 2) = " + deepArrayEqual); // 결과 - true
        System.out.println("deepArrayEqual2 (1 vs 3) = " + deepArrayEqual2); // 결과 - false
        
        // 7. binarySearch(arr, val) 메소드 : arr 배열에서 val 값의 인덱스를 찾음 (단, 배열이 정렬된 상태여야 함)
        int[] sortedArray = { 10, 30, 50, 70, 90, 110, 130, 150 };
        int index = Arrays.binarySearch(sortedArray, 70);
        System.out.println("Index of 70 = " + index); // 결과 - 3
