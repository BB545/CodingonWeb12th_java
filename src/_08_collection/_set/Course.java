...
public class Course {
    ...
    // - hashCode() 메소드: Object 클래스에 정의되어 있으며, 객체의 해시코드를 반환 가능
    // => 이 클래스에서 hashCode() 재정의: 객체 내부 상태를 기반으로 한 고유한 해시코드 생성할 것 (내가 원하는 해시코드로 커스터마이징)
    @Override
    public int hashCode() {
        // title 필드의 타입은 String 이므로 hashCode() 메소드 사용
        // id 는 int 타입이므로 정수 값의 해시코드에 더하기 연산
        
        // => course 인스턴스의 title, id 가 같아면 동일한 해시코드를 반환하도록
